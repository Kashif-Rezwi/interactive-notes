...
| | Interactive notes — **final version** (CAN-2026-0003, v4; non-independent review; not released) | [Open the v4 notes](generated/linear-algebra-foundations-v4.html) |
...
### Version history (historical — not the reference)

The reference entry point for each class is the **final version** in the Classes and material table above. The table below is preserved for provenance only: history of this module's Class 1 interactive notes, in generation order. Governed rebuilds are named `<note-slug>-v<N>.html`, where the slug is derived from the source note's title and `<N>` is the ordinal rebuild version (see [Naming conventions](../../../docs/10-governance/naming-conventions.md)). The filename version is a display version; the stable identity of each governed candidate is its CAN identifier, recorded in the artifact's provenance header and run ledger.
