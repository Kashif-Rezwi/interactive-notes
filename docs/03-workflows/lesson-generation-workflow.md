...
- **Do:** generate the candidate with the versioned prompt card ([PRM-generator-lesson-standard](../../library/prompts/prm-generator-lesson-standard@0.4.0.md)); record the run ledger (RUN) with pinned input identities, prompt digest, **full in-repo prompt snapshot ([prompt-architecture.md](../05-prompts/prompt-architecture.md); WF-001/WF-015)**, and verification evidence; **record iteration counts exactly per [ADR-0006](../../docs/adr/0006-record-iteration-accounting.md)** (generation iterations, in-generation corrections, revision cycles). **The artifact must conform to the XS element-for-element at the specified depth; a specified element that cannot be built well is escalated to the plan owner, never silently dropped (prompt items 10–13).**
...
| 5 | **Technical & behavioral** | Does the code and logic work under simulation? | Syntax/structure/duplicate-ID scans; zero-dependency scan; **handler-level behavioral simulation** (gates, grading, feedback, state); measured contrast; keyboard/no-JS/reduced-motion/print review |
