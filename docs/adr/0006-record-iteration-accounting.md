...
1. **Run ledgers** must declare `Iteration counts` with three distinct, defined counters:
   ...
   - *in-generation corrections* — assembly/fix passes before evaluation;
...
4. Existing records are **not retroactively rewritten**. Where real counts can be reconstructed, a dated retrospective appendix may document them (append-only-legal). The field is mandatory for records created after 2026-08-11.
5. This changes template fields only; it does not change any gate, score, or traceability identity of artifacts.
...
Demonstrated retrospectively on RUN-20260810-0001 and EVAL-2026-0002 (dated appendices, 2026-08-11): generation = 1, in-generation corrections = 3, revision cycles = 1; EVAL reviewed 2 builds / 1 revision cycle. Validated formally by the next governed run.
