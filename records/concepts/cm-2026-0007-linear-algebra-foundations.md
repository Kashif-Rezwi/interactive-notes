# CM-2026-0007: Mathematical foundations and linear algebra for AIML-4 Module 2 (v10 full-verification reproduction run)

**Status:** Reviewed<br>
**Supersedes / iteration position:** Iteration 5 — supersedes [CM-2026-0006](../concepts/cm-2026-0006-linear-algebra-foundations.md) (the source SRC-2026-0001 is byte-unchanged and the active depth bar is unchanged; this iteration exists because RUN-20260904-0001 executes the pipeline with complete live rendered-output verification (ADR-0010 Audit 6), which RUN-20260903-0001 could not perform in degraded mode — no concept, claim, or misconception changes hands)<br>
**Owner:** Repository maintainer (solo Stage 1 operator, Creator profile)<br>
**Source package:** [SRC-2026-0001](../sources/src-2026-0001-aiml-4-module-02.md) — SHA-256 `23c6f4ebe147e63db7adb5f6aa04e773d66bdf02a82f80605f9d8e1611f94445`, 63 markdown cells (hash re-verified at intake, 2026-09-04)<br>
...
Covers all seven parts of the source notebook:
1. Notation (scalars, vectors, matrices, functions, summation, notation kit, proof intuition)
2. Vector spaces and geometry (vector space, linear combination, span, basis, dimension)
3. Core operations (dot product, L2/L1 norms, orthogonality)
4. Projections and least squares (projection, orthogonal decomposition, normal equation)
5. Matrices and transformations (multiplication, matrix-vector product, identity, transpose, symmetry)
6. Rank and linear independence
7. ML-connections table
...
40 anchored atomic claims across the 27 concepts above, carried 1:1 from CM-2026-0005 (reproducing CM-2026-0003's validated depth floor) — including the single-mention load-bearing claims (cells 9, 12, 19, 21, 35) that a thin claim list would otherwise starve (MEM-2026-0004). Every claim carries a one-based cell-ordinal anchor to SRC-2026-0001 in the rightmost column; no claim exceeds the source; constructed examples/non-examples are labeled in-artifact.

## Prerequisites and relationships

Carried 1:1 from CM-2026-0005. Full ordering (empty-taught-set read-through passes):
scalar → vector → matrix → function/linear model → summation → notation kit → dot-product linearity proof → vector space → linear combination → span → basis → dimension → dot product/orthogonality → norms → projection → orthogonal decomposition → normal equation (requires the added inverse + cos θ FOUNDATION bridges) → matrix operations → matrix–vector product → identity → transpose → symmetry → linear independence → rank → ML map.

Source ordering defects repaired and labeled:
- **R1:** Independence/basis moved early (source cell 59 before cell 26)
- **R2:** Matrices moved before projections so Aᵀ in the normal equation is defined before use (source cell 48 before cell 45)
- **R3:** Cos θ primer added before geometric dot product (source cell 30)

## Examples, non-examples, and misconceptions

**Examples:**
- Height/weight/age vector [170,65,25]ᵀ (cell 7)
- 3×2 dataset matrix (cell 10)
- Σᵢ₌₁⁴ xᵢ = x₁+x₂+x₃+x₄ (cell 15)
- (x+y)·z expansion proof (cell 19)
- Standard basis {(1,0),(0,1)} of ℝ² (cell 26)

**Non-examples:**
- A parallel pair of 2D vectors that fail to span the plane
- [1,2] and [2,4] as a dependent pair
- AB ≠ BA on a computed pair
- A 3×2 matrix that cannot have rank 3
- A set containing the zero vector being dependent

**Misconceptions (M1–M15):**
...
- **M5 Span:** "any two 2D vectors span the whole plane" — learner answers "the plane" for a parallel pair.
...
- **M9 Dot product:** "a bigger dot product always means more similar direction" — magnitude confound (‖x‖‖y‖ grows product without changing θ).
...
- **M12 Projection:** "the leftover e can point in any direction" — learner picks a non-perpendicular leftover.
...
- **M15 Multicollinearity:** "a duplicate feature is harmless" — learner sees no problem with column 2 = 2 × column 1.

## Ambiguities, gaps, and assumptions

Identical to CM-2026-0005: cell 2 "Scaler" typo corrected and flagged; cells 13/17/30/59 mangled fragments adopt the standard reading, each flagged; cells 38/41–42 opaque PNGs transcribed, never redistributed; cell 45's normal equation written x̂ with exact-vs-approximate distinction flagged; constructed examples labeled; inverse (·)⁻¹ and cos θ require minimal FOUNDATION bridges; eigenvectors and attention pointers labeled EXTENSION.

## Review and acceptance criteria

- Every concept and claim carries a cell-ordinal anchor to SRC-2026-0001.
- Depth floor exercised: 40 anchored claims across 27 concepts.
- Accepted for Stage 1 governed generation under RUN-20260904-0001; non-independent review limits downstream release eligibility (ADR-0003).

## Conformance checklist (depth-calibration contract)

- [x] ≥ 1 anchored atomic claim per concept (40 claims across 27 concepts)
- [x] Full dependency graph covering every prerequisite and flagging every source use-before-define case (R1–R3 carried)
- [x] ≥ 1 diagnosed misconception per major concept with clear wrong-answer definitions (M1–M15 carried)
- [x] Every source example anchored to cell/section (carried)
- [x] ≥ 1 non-example per conceptual distinction the source draws (carried)
